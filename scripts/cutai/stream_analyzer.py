...
def _cleanup(stream_dir,keep_from):
    ...
    if removed:_log(f"buffer limpo: {removed} segmento(s), {freed/1024/1024:.1f} MiB liberados")
def _cleanup_temp(root):
    if not root.exists():return
    for p in list(root.glob("window-*.mkv"))+list(root.glob("window-*.txt")):
        try:p.unlink()
        except OSError:pass
...
def _deduplicate(clips,analysis_dir,workdir):
    ...
    for clip in clips:
        transcript=getattr(clip,"transcript","") or "";duplicate=next((old for old in history if _similar(transcript,old.get("transcript",""))>=0.72),None)
        if duplicate:
            removed.append(clip.id)
            for suffix in (".mp4",".jpg",".captions.json"):(analysis_dir/f"{clip.id}{suffix}").unlink(missing_ok=True)
        else:kept.append(clip);history.append({"id":clip.id,"transcript":transcript,"created_at":getattr(clip,"created_at","")})
...
def run(url,stream_dir,workdir,segment_seconds=30,window_seconds=600,overlap_seconds=90,poll_seconds=5,stop_file=None):
    ...
    while True:
        ...
        now=time.monotonic()
        if not worked and now-last_report>=30:_log(f"aguardando buffer: {len(_available(stream_dir,next_segment))}/{needed} segmentos disponíveis (próximo={next_segment})");last_report=now
