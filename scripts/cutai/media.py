...
def make_clip_range(source: Path,output: Path,start: float,end: float) -> None:
    """Gera MP4 reproduzível mesmo quando HLS trouxe descontinuidades de timestamp."""
    start=max(0.0,start);length=max(1.0,end-start);output.parent.mkdir(parents=True,exist_ok=True)
    # -ss depois do input evita seek baseado em timestamps HLS quebrados. setpts/asetpts
    # recriam a timeline a partir de zero antes do encode, eliminando DTS/PTS enormes.
    command=["ffmpeg","-y","-fflags","+genpts+discardcorrupt","-err_detect","ignore_err","-i",str(source),"-ss",str(start),"-t",str(length),"-map","0:v:0","-map","0:a:0?","-vf","setpts=PTS-STARTPTS","-c:v","libx264","-preset","veryfast","-crf","18","-profile:v","high","-pix_fmt","yuv420p","-fps_mode:v","vfr"]
    # A faixa de áudio é opcional. Quando existe, normalizamos seus timestamps também.
...
""";events=[]
    ...
    ass_path.write_text(header+"\n".join(events)+"\n",encoding="utf-8");subtitle_filter=f"ass=filename='{ass_path.as_posix()}'";run(["ffmpeg","-y","-i",str(source),"-vf",subtitle_filter,"-map","0:v:0","-map","0:a:0?","-c:v","libx264","-preset","veryfast","-crf","18","-profile:v","high","-pix_fmt","yuv420p","-fps_mode:v","vfr","-c:a","copy","-movflags","+faststart","-shortest",str(output)]);ass_path.unlink(missing_ok=True)
...
def thumbnail(source: Path,output: Path) -> None:run(["ffmpeg","-y","-i",str(source),"-vf","select=gte(t\\,2),scale=640:-2","-frames:v","1",str(output)])
