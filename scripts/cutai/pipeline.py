...
def _looks_complete(text: str) -> bool:
    ...
    if not words or words[-1] in CONTINUATION_ENDINGS: return False
...
def _has_conclusion(text: str) -> bool:
    ...
    tail = words[-20:]
    return _looks_complete(text) and (any(term in tail for term in CONCLUSION_TERMS) or text.endswith(("!", "?")))
...
def _topic_blocks(segments: list[dict], source_duration: float) -> list[dict]:
    ...
    for block in blocks:
        if merged and block["end"] - block["start"] < 12.0 and block["start"] - merged[-1]["end"] < 3.0:
            merged[-1]["end"] = block["end"]; merged[-1]["segments"].extend(block["segments"]); merged[-1]["text"] += " " + block["text"]
        else: merged.append(block)
    return merged


def _block_score(block: dict) -> float:
    text = block["text"]; normalized = text.lower(); words = re.findall(r"[\wÀ-ÿ]+", normalized)
    ...
    score = min(30.0, len(words) * 0.13) + min(25.0, reaction_hits * 7.0 + text.count("!") * 2.0) + min(25.0, story_hits * 2.8 + text.count("?") * 2.5) + min(15.0, duration_s * 0.25)
    if _looks_complete(text): score += 5.0
...
def _context_range(blocks: list[dict], index: int, source_duration: float) -> tuple[float, float]:
    ...
    # Depois do mínimo, procure explicitamente um desfecho natural. Uma simples
    # pontuação não basta: o corte deve avançar até conclusão, pausa real ou troca de assunto.
    while cursor < len(blocks) and end - start < MAX_CLIP_SECONDS:
        ...
        if _has_conclusion(current_text) and gap >= 0.6: break
    ...
    if end - start < MIN_CLIP_SECONDS:
        pad = MIN_CLIP_SECONDS - (end - start); start = max(0.0, start - min(pad, 12.0)); end = min(source_duration, max(end, start + MIN_CLIP_SECONDS))
...
def choose_top_ranges(segments: list[dict], source_duration: float, limit: int = 3) -> list[tuple[float, float, float]]:
    ...
    for index, block in enumerate(blocks):
        start, end = _context_range(blocks, index, source_duration); candidates.append((_block_score(block), start, end))
    ...
    for candidate in candidates:
        ...
        for _, chosen_start, chosen_end in selected:
            intersection = max(0.0, min(end, chosen_end) - max(start, chosen_start)); smaller = max(1.0, min(end - start, chosen_end - chosen_start))
            if intersection / smaller > 0.25: overlap = True; break
        if not overlap:
...
def process(url: str, workdir: Path, capture_seconds: int = 180) -> list[Clip]:
    ...
    _full_transcript, segments = transcribe(source); ranges = choose_top_ranges(segments, source_duration, limit=3); clips = []
    for rank, (context_score, start, end) in enumerate(ranges, start=1):
        ...
        raw_clip_path = workdir / f"{clip_id}.raw.mp4"; clip_path = workdir / f"{clip_id}.mp4"; thumb_path = workdir / f"{clip_id}.jpg"
        make_clip_range(source, raw_clip_path, start, end)
        local_segments = []
        for segment in segments:
            seg_start = float(segment.get("start", 0)); seg_end = float(segment.get("end", 0))
            if seg_end > start and seg_start < end: local_segments.append({"start": max(0.0, seg_start - start), "end": min(end - start, seg_end - start), "text": str(segment.get("text", ""))})
        burn_subtitles(raw_clip_path, clip_path, local_segments); raw_clip_path.unlink(missing_ok=True); thumbnail(clip_path, thumb_path)
        transcript = " ".join(s["text"] for s in local_segments).strip(); mean, peak = audio_metrics(clip_path); a_score, a_reasons = audio_score(mean, peak); t_score, t_reasons = transcript_score(transcript); s_score = scene_score(clip_path); base_score = combine_scores(a_score, t_score, s_score); base_score.total = round(min(100.0, base_score.total * 0.65 + context_score * 0.35), 2); description, hashtags = suggest_metadata(transcript)
        reasons = a_reasons + t_reasons + [f"contexto {context_score:.0f}/100", f"cena {s_score:.0f}/100", f"duração {end-start:.1f}s", "bloco completo com desfecho natural", "legendado"]
        score_breakdown = {"audio": round(a_score, 2), "transcript": round(t_score, 2), "scene": round(s_score, 2), "context": round(context_score, 2)}
        clip = Clip(id=clip_id, title=source_title, source_title=source_title, score=base_score, score_breakdown=score_breakdown, duration=round(end-start, 2), source_url=url, asset_url="", thumbnail_url="", transcript=transcript, reasons=reasons, description=description, hashtags=hashtags, created_at=datetime.now(UTC).isoformat()); upsert_clip(RANKING_PATH, clip); clips.append(clip)
    clips.sort(key=lambda c: c.score.total, reverse=True); return clips[:3]


def main() -> None:
    parser = argparse.ArgumentParser(); parser.add_argument("--url", required=True); parser.add_argument("--capture-seconds", type=int, default=180); parser.add_argument("--workdir", default="work"); args = parser.parse_args(); clips = process(args.url, Path(args.workdir), args.capture_seconds); Path(args.workdir, "result.json").write_text(json.dumps({"clips": [clip.to_dict() for clip in clips]}, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
