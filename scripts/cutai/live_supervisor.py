...
def _probe_live(url,timeout=45):
    cmd=["yt-dlp","--no-playlist","--no-progress","--skip-download"]
    try:proxy=normalize_proxy_url(os.getenv("CUTAI_PROXY_URL",""))
    except ValueError:return "unknown"
    if proxy:cmd += ["--proxy",proxy]
    cmd += ["--print","%(live_status)s",url]
...
def run(url,root,segment=30,window=600,overlap=90,restarts=12):
    ...
    signal.signal(signal.SIGINT,request);signal.signal(signal.SIGTERM,request);analyzer=subprocess.Popen(_analyzer_cmd(url,stream,analysis,segment,window,overlap,stop));_write(state,status="starting",url=url,started_at=started_at,capture_restarts=0,analyzer_pid=analyzer.pid,proxy_enabled=proxy_enabled)
    try:
        while not shutdown:
            if analyzer.poll() is not None:analyzer=subprocess.Popen(_analyzer_cmd(url,stream,analysis,segment,window,overlap,stop))
            capture_started=time.monotonic();capture=subprocess.Popen(_capture_cmd(url,stream,segment));_write(state,status="capturing",url=url,started_at=started_at,capture_restarts=disconnects,capture_pid=capture.pid,analyzer_pid=analyzer.pid,connected_at=_now(),end_confirmations=ended_confirmations,proxy_enabled=proxy_enabled)
