...
def _probe_live(url,timeout=45):
    ...
    try:r=subprocess.run(cmd,text=True,capture_output=True,timeout=timeout);out=(r.stdout+"\n"+r.stderr).lower()
...
def run(url,root,segment=30,window=600,overlap=90,restarts=12):
    ...
    signal.signal(signal.SIGINT,request);signal.signal(signal.SIGTERM,request);analyzer=subprocess.Popen(_analyzer_cmd(url,stream,analysis,segment,window,overlap,stop));_write(state,status="starting",url=url,started_at=started_at,capture_restarts=0,analyzer_pid=analyzer.pid)
    try:
        while not shutdown:
            ...
            # Erro de bot/rate-limit/rede é estado desconhecido, não fim da live. Não
            # encerramos mais a sessão por quantidade de falhas: mantemos o worker
            # aguardando e tentamos novamente com backoff até a fonte voltar ou o
            # usuário mandar parar.
            if probe=="unknown":
                backoff=min(120,max(5,backoff*2));_write(state,status="waiting_source",url=url,started_at=started_at,capture_restarts=disconnects,consecutive_failures=consecutive_failures,live_probe=probe,retry_in_seconds=backoff,analyzer_pid=analyzer.pid if analyzer.poll() is None else None);time.sleep(backoff);continue
